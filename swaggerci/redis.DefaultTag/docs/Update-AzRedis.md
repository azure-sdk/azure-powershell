...
Update-AzRedis -InputObject <IRedisIdentity> [-EnableNonSslPort] [-IdentityType <ManagedServiceIdentityType>]
 ...
 [-TenantSetting <Hashtable>] [-DefaultProfile <PSObject>] [-AsJob] [-NoWait] [-Confirm] [-WhatIf]
 [<CommonParameters>]
```
...
```yaml
Type: Microsoft.Azure.PowerShell.Cmdlets.Redis.Models.Api20220601Privatepreview.IRedisCommonPropertiesRedisConfiguration
Parameter Sets: (All)
Aliases:
...
Valid values: (C, P).
(C = Basic/Standard, P = Premium, G = General Purpose, M = Memory Optimized).
...
Valid values: (Basic, Standard, Premium, General Purpose, Memory Optimized)
