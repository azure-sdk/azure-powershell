...
MAPSACCOUNTSASPARAMETER <IAccountSasParameters>: Parameters used to create an account Shared Access Signature (SAS) token. The REST API access control is provided by Azure Maps Role Based Access (RBAC) identity and access.
  ...
  SigningKey <SigningKey>: The Map account key to use for signing. Picking `primaryKey` or `secondaryKey` will use the Map account Shared Keys, and using `managedIdentity` will use the auto-renewed private key to sign the SAS.
...
PROPERTIESLOCATIONS <ILocationsItem[]>: List of enabled data processing locations for the Maps Account. If no locations are set, Azure REST APIs will only enable features available in the Maps Account's location.
