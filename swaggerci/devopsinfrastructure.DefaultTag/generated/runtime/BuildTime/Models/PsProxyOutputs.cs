        ...
        private string GetDefaultValuesStatements()
        {
            ...
            foreach (var defaultInfo in defaultInfos)
            {
                ...
                if ("SubscriptionId".Equals(parameterName))
                {
                    ...
                    sb.AppendLine($"{Indent}{Indent}{Indent}$PSBoundParameters['HttpPipelinePrepend'] | Foreach-Object {{ $testPlayback = $testPlayback -or ('Microsoft.Azure.PowerShell.Cmdlets.ManagedDevOpsInfrastructure.Runtime.PipelineMock' -eq $_.Target.GetType().FullName) }}");
