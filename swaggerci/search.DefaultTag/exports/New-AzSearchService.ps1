...
NETWORKRULESETIPRULE <IIPRule[]>: A list of IP restriction rules used for an IP firewall. Any IPs that do not match the rules are blocked by the firewall. These rules are only applied when the 'publicNetworkAccess' of the search service is 'enabled'.
  [Value <String>]: Value corresponding to a single IPv4 address (for example, 123.1.2.3) or an IP range in CIDR format (for example, 123.1.2.3/24) to be allowed.
.Link
...
param(
    ...
    # A list of IP restriction rules used for an IP firewall.
    # Any IPs that do not match the rules are blocked by the firewall.
