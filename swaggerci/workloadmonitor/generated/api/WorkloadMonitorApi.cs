    ...
    public partial class WorkloadMonitorApi
    {
        ...
        internal async global::System.Threading.Tasks.Task HealthMonitorsGetStateChange_Validate(string subscriptionId, string resourceGroupName, string providerName, string resourceCollectionName, string resourceName, string monitorId, string timestampUnix, string Expand, Microsoft.Azure.PowerShell.Cmdlets.WorkloadMonitorApi.Runtime.IEventListener eventListener)
        {
            using( NoSynchronizationContext )
            {
                ...
                await eventListener.AssertMinimumLength(nameof(providerName),providerName,3);
                await eventListener.AssertMaximumLength(nameof(providerName),providerName,63);
                await eventListener.AssertRegEx(nameof(providerName),providerName,@"^[A-Za-z0-9.]*$");
                await eventListener.AssertNotNull(nameof(resourceCollectionName),resourceCollectionName);
                await eventListener.AssertMinimumLength(nameof(resourceCollectionName),resourceCollectionName,3);
                await eventListener.AssertMaximumLength(nameof(resourceCollectionName),resourceCollectionName,63);
                await eventListener.AssertRegEx(nameof(resourceCollectionName),resourceCollectionName,@"^[A-Za-z0-9]*$");
