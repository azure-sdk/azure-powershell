# How-To
This document describes how to develop for `Az.MachineLearningCompute`.

## Building `Az.MachineLearningCompute`
...
## Testing `Az.MachineLearningCompute`
...
## Packing `Az.MachineLearningCompute`
...
There are multiple scripts created for performing different actions for developing `Az.MachineLearningCompute`.
- `build-module.ps1`
  - Builds the module DLL (`./bin/Az.MachineLearningCompute.private.dll`), creates the exported cmdlets and documentation, generates custom cmdlet test stubs and exported cmdlet example stubs, and updates `./Az.MachineLearningCompute.psd1` with Azure profile information.
  - **Parameters**: [`Switch` parameters]
    - `-Run`: After building, creates an isolated PowerShell session and loads `Az.MachineLearningCompute`.
...
- `run-module.ps1`
  - Creates an isolated PowerShell session and loads `Az.MachineLearningCompute` into the session.
