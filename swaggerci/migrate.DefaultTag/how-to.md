# How-To
This document describes how to develop for `Az.MigrateEngineApIs`.

## Building `Az.MigrateEngineApIs`
...
There are multiple scripts created for performing different actions for developing `Az.MigrateEngineApIs`.
- `build-module.ps1`
  - Builds the module DLL (`./bin/Az.MigrateEngineApIs.private.dll`), creates the exported cmdlets and documentation, generates custom cmdlet test stubs and exported cmdlet example stubs, and updates `./Az.MigrateEngineApIs.psd1` with Azure profile information.
  - **Parameters**: [`Switch` parameters]
    - `-Run`: After building, creates an isolated PowerShell session and loads `Az.MigrateEngineApIs`.
...
- `run-module.ps1`
  - Creates an isolated PowerShell session and loads `Az.MigrateEngineApIs` into the session.
