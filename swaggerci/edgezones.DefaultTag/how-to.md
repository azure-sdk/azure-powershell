# How-To
This document describes how to develop for `Az.EdgeZonesTest`.

## Building `Az.EdgeZonesTest`
...
There are multiple scripts created for performing different actions for developing `Az.EdgeZonesTest`.
- `build-module.ps1`
  - Builds the module DLL (`./bin/Az.EdgeZonesTest.private.dll`), creates the exported cmdlets and documentation, generates custom cmdlet test stubs and exported cmdlet example stubs, and updates `./Az.EdgeZonesTest.psd1` with Azure profile information.
  - **Parameters**: [`Switch` parameters]
    - `-Run`: After building, creates an isolated PowerShell session and loads `Az.EdgeZonesTest`.
...
- `run-module.ps1`
  - Creates an isolated PowerShell session and loads `Az.EdgeZonesTest` into the session.
