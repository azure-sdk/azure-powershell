# How-To
This document describes how to develop for `Az.DatabaseWatcherTest`.

## Building `Az.DatabaseWatcherTest`
...
## Testing `Az.DatabaseWatcherTest`
...
## Packing `Az.DatabaseWatcherTest`
To pack `Az.DatabaseWatcherTest` for distribution, run the `pack-module.ps1` script. This will take the contents of multiple directories and certain root-folder files to create a `.nupkg`. The structure of the `.nupkg` is created so it can be loaded part of a [PSRepository](https://learn.microsoft.com/powershell/module/powershellget/register-psrepository). Additionally, this package is in a format for distribution to the [PSGallery](https://www.powershellgallery.com/). For signing an Azure module, please contact the [Azure PowerShell](https://github.com/Azure/azure-powershell) team.

## Module Script Details
There are multiple scripts created for performing different actions for developing `Az.DatabaseWatcherTest`.
- `build-module.ps1`
  - Builds the module DLL (`./bin/Az.DatabaseWatcherTest.private.dll`), creates the exported cmdlets and documentation, generates custom cmdlet test stubs and exported cmdlet example stubs, and updates `./Az.DatabaseWatcherTest.psd1` with Azure profile information.
  - **Parameters**: [`Switch` parameters]
    - `-Run`: After building, creates an isolated PowerShell session and loads `Az.DatabaseWatcherTest`.
...
- `run-module.ps1`
  - Creates an isolated PowerShell session and loads `Az.DatabaseWatcherTest` into the session.
