# How-To
This document describes how to develop for `Az.DatabaseWatcher`.

## Building `Az.DatabaseWatcher`
...
There are multiple scripts created for performing different actions for developing `Az.DatabaseWatcher`.
- `build-module.ps1`
  - Builds the module DLL (`./bin/Az.DatabaseWatcher.private.dll`), creates the exported cmdlets and documentation, generates custom cmdlet test stubs and exported cmdlet example stubs, and updates `./Az.DatabaseWatcher.psd1` with Azure profile information.
  - **Parameters**: [`Switch` parameters]
    - `-Run`: After building, creates an isolated PowerShell session and loads `Az.DatabaseWatcher`.
...
- `run-module.ps1`
  - Creates an isolated PowerShell session and loads `Az.DatabaseWatcher` into the session.
