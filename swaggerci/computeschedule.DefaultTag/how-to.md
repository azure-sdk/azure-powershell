# How-To
This document describes how to develop for `Az.ComputeScheduleTest`.

## Building `Az.ComputeScheduleTest`
...
## Testing `Az.ComputeScheduleTest`
...
## Packing `Az.ComputeScheduleTest`
...
There are multiple scripts created for performing different actions for developing `Az.ComputeScheduleTest`.
- `build-module.ps1`
  - Builds the module DLL (`./bin/Az.ComputeScheduleTest.private.dll`), creates the exported cmdlets and documentation, generates custom cmdlet test stubs and exported cmdlet example stubs, and updates `./Az.ComputeScheduleTest.psd1` with Azure profile information.
  - **Parameters**: [`Switch` parameters]
    - `-Run`: After building, creates an isolated PowerShell session and loads `Az.ComputeScheduleTest`.
...
- `run-module.ps1`
  - Creates an isolated PowerShell session and loads `Az.ComputeScheduleTest` into the session.
