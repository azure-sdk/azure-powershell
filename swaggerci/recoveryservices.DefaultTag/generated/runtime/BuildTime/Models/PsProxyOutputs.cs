        ...
        private string GetDefaultValuesStatements()
        {
            ...
            foreach (var defaultInfo in defaultInfos)
            {
                ...
                if ("SubscriptionId".Equals(parameterName))
                {
                    ...
                    sb.AppendLine($"{Indent}{Indent}{Indent}$PSBoundParameters['HttpPipelinePrepend'] | Foreach-Object {{ if ($_) {{ $testPlayback = $testPlayback -or ('Microsoft.Azure.PowerShell.Cmdlets.RecoveryServices.Runtime.PipelineMock' -eq $_.Target.GetType().FullName -and 'Playback' -eq $_.Target.Mode) }} }}");
