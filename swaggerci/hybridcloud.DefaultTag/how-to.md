# How-To
This document describes how to develop for `Az.HybridCloud`.

## Building `Az.HybridCloud`
...
There are multiple scripts created for performing different actions for developing `Az.HybridCloud`.
- `build-module.ps1`
  - Builds the module DLL (`./bin/Az.HybridCloud.private.dll`), creates the exported cmdlets and documentation, generates custom cmdlet test stubs and exported cmdlet example stubs, and updates `./Az.HybridCloud.psd1` with Azure profile information.
  - **Parameters**: [`Switch` parameters]
    - `-Run`: After building, creates an isolated PowerShell session and loads `Az.HybridCloud`.
...
- `run-module.ps1`
  - Creates an isolated PowerShell session and loads `Az.HybridCloud` into the session.
