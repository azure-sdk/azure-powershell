# How-To
This document describes how to develop for `Az.RedHatOpenShift4`.

## Building `Az.RedHatOpenShift4`
...
## Packing `Az.RedHatOpenShift4`
To pack `Az.RedHatOpenShift4` for distribution, run the `pack-module.ps1` script. This will take the contents of multiple directories and certain root-folder files to create a `.nupkg`. The structure of the `.nupkg` is created so it can be loaded part of a [PSRepository](https://docs.microsoft.com/powershell/module/powershellget/register-psrepository). Additionally, this package is in a format for distribution to the [PSGallery](https://www.powershellgallery.com/). For signing an Azure module, please contact the [Azure PowerShell](https://github.com/Azure/azure-powershell) team.

## Module Script Details
There are multiple scripts created for performing different actions for developing `Az.RedHatOpenShift4`.
- `build-module.ps1`
  - Builds the module DLL (`./bin/Az.RedHatOpenShift4.private.dll`), creates the exported cmdlets and documentation, generates custom cmdlet test stubs and exported cmdlet example stubs, and updates `./Az.RedHatOpenShift4.psd1` with Azure profile information.
  - **Parameters**: [`Switch` parameters]
    - `-Run`: After building, creates an isolated PowerShell session and loads `Az.RedHatOpenShift4`.
...
- `run-module.ps1`
  - Creates an isolated PowerShell session and loads `Az.RedHatOpenShift4` into the session.
