# How-To
This document describes how to develop for `Az.MachineLearningWorkspaces`.

## Building `Az.MachineLearningWorkspaces`
...
## Testing `Az.MachineLearningWorkspaces`
...
## Packing `Az.MachineLearningWorkspaces`
...
There are multiple scripts created for performing different actions for developing `Az.MachineLearningWorkspaces`.
- `build-module.ps1`
  - Builds the module DLL (`./bin/Az.MachineLearningWorkspaces.private.dll`), creates the exported cmdlets and documentation, generates custom cmdlet test stubs and exported cmdlet example stubs, and updates `./Az.MachineLearningWorkspaces.psd1` with Azure profile information.
  - **Parameters**: [`Switch` parameters]
    - `-Run`: After building, creates an isolated PowerShell session and loads `Az.MachineLearningWorkspaces`.
...
- `run-module.ps1`
  - Creates an isolated PowerShell session and loads `Az.MachineLearningWorkspaces` into the session.
