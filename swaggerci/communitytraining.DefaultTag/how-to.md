# How-To
This document describes how to develop for `Az.CommunityForTesting`.

## Building `Az.CommunityForTesting`
...
## Testing `Az.CommunityForTesting`
...
## Packing `Az.CommunityForTesting`
...
There are multiple scripts created for performing different actions for developing `Az.CommunityForTesting`.
- `build-module.ps1`
  - Builds the module DLL (`./bin/Az.CommunityForTesting.private.dll`), creates the exported cmdlets and documentation, generates custom cmdlet test stubs and exported cmdlet example stubs, and updates `./Az.CommunityForTesting.psd1` with Azure profile information.
  - **Parameters**: [`Switch` parameters]
    - `-Run`: After building, creates an isolated PowerShell session and loads `Az.CommunityForTesting`.
...
- `run-module.ps1`
  - Creates an isolated PowerShell session and loads `Az.CommunityForTesting` into the session.
