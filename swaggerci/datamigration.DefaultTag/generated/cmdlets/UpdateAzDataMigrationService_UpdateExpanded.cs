...
namespace Microsoft.Azure.PowerShell.Cmdlets.DataMigration.Cmdlets
{
    ...
    /// currently running (i.e. the service is busy), this will fail with 400 Bad Request (\"ServiceIsBusy\"). Database Migration
    /// Service (classic) - SQL scenarios are on a deprecation path and will be retired on 15 March 2026 for all customers. Please
    /// migrate to Azure SQL database services by using the latest Azure Database Migration Service version which is available
    ...
    [global::Microsoft.Azure.PowerShell.Cmdlets.DataMigration.Description(@"The services resource is the top-level resource that represents the Azure Database Migration Service (classic). The PATCH method updates an existing service. This method can change the kind, SKU, and network of the service, but if tasks are currently running (i.e. the service is busy), this will fail with 400 Bad Request (\""ServiceIsBusy\""). Database Migration Service (classic) - SQL scenarios are on a deprecation path and will be retired on 15 March 2026 for all customers. Please migrate to Azure SQL database services by using the latest Azure Database Migration Service version which is available as an extension in Azure Data Studio, or by using Azure PowerShell and Azure CLI.")]
        ...
        protected async global::System.Threading.Tasks.Task ProcessRecordAsync()
        {
            using( NoSynchronizationContext )
            {
                ...
                try
                {
                    ...
                    await this.Client.ServicesUpdate(SubscriptionId, GroupName, Name, _parametersBody, onOk, onDefault, this, Pipeline);
